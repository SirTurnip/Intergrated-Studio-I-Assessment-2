...
enum Sections { firstName = 0, lastName = 1, gender = 2, id = 3, date = 0, month = 1, year = 2, day = 0, classTime = 0, period = 1, subject = 2, room = 3 };
...
int main() {
    ...
    studentsFile.open("C:/Users/Auren/Documents/Assessment 2/Assessment 2/Students.csv");
    ...
    for(int i = 0; i < content.size(); i++) {
        if(i - sec == 0) { //name
            if(content[i][firstName] == myInput) {
                if(content[i][lastName] == myInput1) {
                    student.MyFunction(i);
                }
            } else if(content[i][id] == myInput) {
                student.MyFunction(i);
            }
        }
